# 💰 Finance Manager

A modern, lightweight **Finance Manager Web Application** that helps users track their **income, savings, and expenses** efficiently with visual charts and smart alerts.

---

## 📌 Overview

Finance Manager is a browser-based personal finance tracker built using HTML, CSS, and JavaScript. It allows users to manage monthly income, monitor spending habits, and stay within budget using real-time insights.

---

## ✨ Key Features

- ✅ Set and update **monthly income**
- 🎯 Define a **savings target (%)**
- 🧾 Add, view, and delete **daily expenses**
- 📊 Visual expense & savings charts
- ⚠️ **Automatic alert** when expenses exceed 80% of income
- 🌓 **Light / Dark theme toggle**
- 💾 Persistent data storage using **LocalStorage**
- 📱 Responsive and user-friendly UI

---


## 🛠️ Tech Stack
- **HTML5**
- **CSS3**
- **JavaScript**
- **Chart.js**
- **Browser LocalStorage**

---

## 📊 Application Modules

- Income & Savings Module
- Expense Management Module
- Expense Table & Deletion
- Data Visualization (Charts)
- Theme Controller
- Budget Alert System
- Theme Controller

---

## 🔮 Future Enhancements

- Export data to CSV / PDF
- User login & authentication
- Cloud database integration
- Monthly & yearly financial reports

---
